...
use corsa_bind_rs::api::{ApiClient, ApiMode, PrintNodeOptions, UpdateSnapshotParams};
...
fn async_api_roundtrip_core() {
    block_on(async {
        ...
        let printed = client
            .print_node(
                &corsa_bind_rs::api::EncodedPayload::new(b"hello".to_vec()),
...
fn async_api_full_surface_methods() {
    block_on(async {
        ...
        let snapshot = client
            ...
            .unwrap();
        let project = snapshot.projects[0].id.clone();
        let node = corsa_bind_rs::api::NodeHandle("1.3.80./workspace/src/index.ts".into());
        let symbol = client
