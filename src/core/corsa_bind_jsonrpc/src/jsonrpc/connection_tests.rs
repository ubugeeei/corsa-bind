...
use corsa_bind_core::{CorsaEvent, CorsaObserver};
use serde_json::json;
...
fn routes_request_and_response() {
    ...
    let response: serde_json::Value =
        corsa_bind_runtime::block_on(client.request("ping", json!({"value": 1}))).unwrap();
    waiter.join().unwrap();
...
fn request_times_out_when_no_response_arrives() {
    ...
    let error = corsa_bind_runtime::block_on(client.request_value("ping", json!({"value": 1})))
        .unwrap_err();
    assert!(matches!(
        error,
