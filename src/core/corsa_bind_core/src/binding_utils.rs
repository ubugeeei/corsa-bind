...
/// JSON-friendly input shared by the binding-level unsafe-type helpers.
#[derive(Debug, Clone, Default, PartialEq, Eq, Deserialize, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct UnsafeTypeFlowInput {
    pub source_type_texts: Vec<String>,
    #[serde(default)]
    pub target_type_texts: Vec<String>,
}

/// Shared binding utility surface backed by Rust core logic.
pub struct CorsaUtils;

impl CorsaUtils {
    /// Returns whether a source type set can flow into the target set through an
    /// `any`-like path.
    pub fn is_unsafe_assignment(input: &UnsafeTypeFlowInput) -> bool {
        has_unsafe_any_flow(&input.source_type_texts, &input.target_type_texts)
    }

    /// Returns whether a return type flow should be treated as unsafe.
    pub fn is_unsafe_return(input: &UnsafeTypeFlowInput) -> bool {
        has_unsafe_any_flow(&input.source_type_texts, &input.target_type_texts)
    }
}
...
#[cfg(test)]
mod tests {
    use super::{CorsaUtils, SimpleType, UnsafeTypeFlowInput, parse_type_text};
    ...
    fn flags_direct_any_assignment() {
        assert!(CorsaUtils::is_unsafe_assignment(&UnsafeTypeFlowInput {
            source_type_texts: vec![String::from("any")],
            target_type_texts: vec![String::from("string")],
        }));
    }
    ...
    fn allows_unknown_targets() {
        assert!(!CorsaUtils::is_unsafe_assignment(&UnsafeTypeFlowInput {
            source_type_texts: vec![String::from("any")],
            target_type_texts: vec![String::from("unknown")],
        }));
    }
    ...
    fn flags_generic_any_assignment() {
        assert!(CorsaUtils::is_unsafe_assignment(&UnsafeTypeFlowInput {
            source_type_texts: vec![String::from("Set<any>")],
            target_type_texts: vec![String::from("Set<string>")],
        }));
    }

    #[test]
    fn flags_promise_any_returns() {
        assert!(CorsaUtils::is_unsafe_return(&UnsafeTypeFlowInput {
            source_type_texts: vec![String::from("Promise<any>")],
            target_type_texts: vec![String::from("Promise<string>")],
        }));
    }
    ...
    fn flags_unions_that_include_any() {
        assert!(CorsaUtils::is_unsafe_assignment(&UnsafeTypeFlowInput {
            source_type_texts: vec![String::from("string | any")],
            target_type_texts: vec![String::from("string")],
        }));
    }
    ...
    fn keeps_specific_flows_allowed() {
        assert!(!CorsaUtils::is_unsafe_assignment(&UnsafeTypeFlowInput {
            source_type_texts: vec![String::from("Promise<string>")],
            target_type_texts: vec![String::from("Promise<string>")],
        }));
    }
}
